...
let storage = multer.diskStorage({
 ...
 //WORKS WITH THIS BUT NOT line 16 instead and not line 22-24
...
module.exports = (app) => {
    ...
    // app.post("/api/user-post", upload.single("file"), (req, res, next) => {

    // app.post("/api/user-post", upload.single("img"), (req, res, next) => {    

    app.post("/api/user-post", upload.single("img"), (req, res) => {
        console.log(req.body.imgFilepath);
        let parsedFile = (req.body.imgFilepath).split("\\");
        console.log("parseFile",parsedFile);        
        // const filePath = `/public/images/${req.file.filename}`;    
        const filePath = `/public/images/${parsedFile[2]}`;   
